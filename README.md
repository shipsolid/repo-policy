...
- **Duplicate YAML keys are rejected** — a custom YAML loader fails a policy file with two `strict:`
  keys, two entries for the same branch name, etc., instead of PyYAML's default silent
...
- `automated_security_fixes: true` requires `vulnerability_alerts: true`, and
  `secret_scanning_push_protection: true` requires `secret_scanning: true` — GitHub rejects
  enabling either one before its prerequisite.
